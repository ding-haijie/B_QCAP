...
public class GeneticAlgorithm {
    ...
    // 遗传算法相关参数
    private static final HashMap<String, Double> propertyMap = readProperties();

    private static final int ITERATION_MAX = propertyMap.get("iteration_max").intValue();// 最大迭代数
    private static final int POPULATION_SIZE = propertyMap.get("population_size").intValue();// 种群大小
    private static final double CROSS_RATE = propertyMap.get("cross_rate");// 交叉率
    private static final double MUTATION_RATE = propertyMap.get("mutation_rate");// 变异率
    private static final int BERTH_NUM = propertyMap.get("berth_num").intValue();// 泊位数
    private static final double CRANE_EFFICIENCY = propertyMap.get("crane_efficiency");// 岸桥工作效率

    // 泊位岸桥分配问题相关参数
    private static final int[] craneMax = loadFileToArray("craneMax");// 能接受的最大岸桥数
    private static final int SHIP_NUM;// 船舶数

    static {
        assert craneMax != null;
        SHIP_NUM = craneMax.length;
    }
    ...
    private int optimalGeneration;// 遗传过程中最优解所在的遗传代数
    private Chromosome optimalChromosome;// 最后一代的最优染色体

    private List<Chromosome> population = new ArrayList<>();// 种群

    private final Map<Integer, Double> bestMap = new HashMap<>();// 保存最好值数据的哈希表
    private final Map<Integer, Double> averageMap = new HashMap<>();// 保存平均值数据的哈希表
    private final Map<Integer, Double> worstMap = new HashMap<>();// 保存最差值数据的哈希表
    ...
    public void start() {
        ...
        while (generation < ITERATION_MAX) {
            ...
            for (Chromosome chromosome : population) {
                setChromosomeScore(chromosome);// 遗传完成后设置新的适应度值
            }
    ...
    private void setChromosomeScore(Chromosome chro) {
        ...
        while (((ships_waitBerth.size() != 0) || (ships_waitCrane.size() != 0) || (ships_service.size() != 0))) {
            ...
            if (ships_waitBerth.size() != 0) {
                ...
                if (berth_free[gene_shipBerth[k] - 1]) {
                    assert time_arrive != null;
                    if (time_arrive[k] <= unitTime) {
                        // 设置泊位为忙碌
...
//            Iterator<Map.Entry<Integer, Boolean>> iterator = ships_service.entrySet().iterator();
...
//            }
            ...
            ships_service.entrySet().removeIf(Map.Entry::getValue);
            unitTime++;
        }
    ...
    private void printCurrentGeneration() {
        ...
        System.out.println("遗传过程中最好适应度值出现在第 " + optimalGeneration + " 代");
        System.out.println("遗传过程中最小在港时间:" + (1 / optimalScore) + "h");
        System.out.println("-----------------------------------------");
    }

    private static HashMap<String, Double> readProperties() {
        ...
        HashMap<String, Double> propertyMap = new HashMap<>();
        propertyMap.put("iteration_max", Double.parseDouble(properties.getProperty("iteration_max")));
        propertyMap.put("population_size", Double.parseDouble(properties.getProperty("population_size")));
        propertyMap.put("cross_rate", Double.parseDouble(properties.getProperty("cross_rate")));
        propertyMap.put("mutation_rate", Double.parseDouble(properties.getProperty("mutation_rate")));
        propertyMap.put("berth_num", Double.parseDouble(properties.getProperty("berth_num")));
        propertyMap.put("crane_efficiency", Double.parseDouble(properties.getProperty("crane_efficiency")));
    ...
    private static int[] loadFileToArray(String str) {
        ...
        try {
            // 用缓冲流按行读取数据
            FileInputStream fis = new FileInputStream(new File("src/main/resources/data"));
